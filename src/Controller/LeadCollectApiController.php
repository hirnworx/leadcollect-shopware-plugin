...
class LeadCollectApiController extends AbstractController
{
     ...
     * Customer info is in sales_channel_api_context -> customer -> customer_address
     * 
     * FIXED in v1.5.0: Now properly joins with customer tables instead of parsing payload
     */
    private function getCartsModern(\DateTime $threshold, int $limit): array
    {
        // Join cart with sales_channel_api_context to get customer_id
        // Then join with customer and customer_address to get full customer data
        $sql = "
            SELECT 
                c.token as cart_token,
                c.payload as cart_payload,
                c.created_at as cart_created_at,
                LOWER(HEX(ctx.customer_id)) as customer_id,
                cu.first_name,
                cu.last_name,
                cu.email,
                ca.street,
                ...
                co.iso as country_iso
            FROM cart c
            INNER JOIN sales_channel_api_context ctx ON c.token = ctx.token
            INNER JOIN customer cu ON ctx.customer_id = cu.id
            LEFT JOIN customer_address ca ON cu.default_billing_address_id = ca.id
            LEFT JOIN country co ON ca.country_id = co.id
            WHERE c.created_at < ?
              AND ctx.customer_id IS NOT NULL
              AND ca.street IS NOT NULL
              AND ca.street != ''
            ORDER BY c.created_at DESC
            LIMIT " . (int)$limit;

        $this->log('info', 'LeadCollect API: Executing modern cart query (v1.5.0)', [
        ...
        $this->log('info', 'LeadCollect API: Raw carts from DB', ['count' => count($rawCarts)]);

        foreach ($rawCarts as $cart) {
            // Parse line items from payload
            $cartData = $this->decodePayload($cart['cart_payload']);
            $lineItems = [];
            
            // Handle Shopware Cart object
            if (is_object($cartData)) {
                $cartData = $this->convertCartObjectToArray($cartData);
            }
            
            if (is_array($cartData)) {
                $lineItems = $this->parseLineItems($cartData);
            }

            // Skip carts without products
            if (empty($lineItems)) {
                $this->log('debug', 'Skipping cart without line items', ['token' => substr($cart['cart_token'], 0, 8)]);
                continue;
            }

            // Calculate cart total from line items if not available
            $cartTotal = 0;
            foreach ($lineItems as $item) {
                $cartTotal += ($item['price'] ?? 0) * ($item['quantity'] ?? 1);
            }

            // Also try to get total from payload
            if (isset($cartData['price']['totalPrice'])) {
                $cartTotal = (float) $cartData['price']['totalPrice'];
            }

            $formattedCarts[] = [
                'cartToken' => strtolower($cart['cart_token']),
                'cartTotal' => $cartTotal,
        ...
        $this->log('info', 'LeadCollect API: Formatted carts', ['count' => count($formattedCarts)]);
    ...
    public function health(): JsonResponse
    {
        ...
        return new JsonResponse([
            ...
            'fix' => 'v1.5.0: Fixed customer data retrieval for Shopware 6.6+ via JOIN instead of payload parsing',
